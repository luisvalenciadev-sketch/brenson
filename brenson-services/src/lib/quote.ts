...
<h2>Dimensiones y carga</h2><table>${row('Peso', mf.peso_kg, ' kg')}${row('Carga máxima', mf.capacidad_carga_kg, ' kg')}${(safe(mf.specs_adicionales) as any[] || []).map((r) => row(r.label, r.value)).join('')}</table>
<h2>Normativa</h2><table>${row('Requiere licencia', mf.requiere_licencia === 'true' ? 'Sí' : 'No')}${row('Requiere SOAT', mf.requiere_soat === 'true' ? 'Sí' : 'No')}${row('Garantía vehículo', mf.garantia_meses, ' meses')}</table>
