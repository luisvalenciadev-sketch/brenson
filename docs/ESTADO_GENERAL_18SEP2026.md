...
| 2c | **Turnstile en los formularios**: el cableado quedó correcto el 18-sep (el JS ya envía `cf-turnstile-response`), pero falta `TURNSTILE_SECRET` en el worker y `brenson_turnstile_site_key` en los ajustes del tema. Mientras tanto solo protege el rate limit de `/b2b/request` (5 solicitudes por IP cada hora). **Ojo al activarlo**: si se pone el secreto en el worker sin la site key en el tema, el widget no se renderiza, no se envía token y el endpoint rechaza el 100% de las solicitudes con 403 — hay que activar los dos lados a la vez | Claves de Cloudflare Turnstile |
...
| 2h | **El ciclo comercial se corta en la cotización: no hay forma de convertirla en pedido.** Verificado: cero referencias a `draftOrder`/`orderCreate` en todo el repo, "Pedidos" del menú apunta al portal alojado de Shopify (siempre vacío si nunca se crea un pedido), la Función de descuento por tier **no está desplegada** (`automaticDiscountNodes` = 0, así que una empresa que arme un carrito paga precio público), y el estado `aceptada` se pinta pero nada lo asigna nunca. Análisis completo, las dos opciones de cierre y lo que se construye según cada una: **[FLUJO_COTIZACION_A_PEDIDO.md](FLUJO_COTIZACION_A_PEDIDO.md)** | **Decisión de negocio de Brenson**: ¿cierra el asesor con pedido borrador, o compra la empresa sola desde el portal? |
| 2i | **Totales de cotización se mostraban en $0** (corregido el 18-sep). El dato guardado era correcto; el render usaba `q.total.value.amount`, la forma del API de Admin, que en Liquid puede no responder → `nil \| times: 100` = 0. Resuelto con el snippet `brenson-money-metafield`. **Queda el mismo patrón latente** en `brenson-price.liquid:45` para `cuota_desde_override`: hoy ningún producto usa ese metafield, pero si se llena mostraría "$0/mes" | Solo si se empieza a usar ese metafield |
